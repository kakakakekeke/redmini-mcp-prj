...
1. [/] `feature/smart-name-resolver` : Smart Name Resolver (상태, 트래커 이름 -> ID 매핑) 로직 구현
2. [/] `feature/get-projects` : get_projects 도구 TDD 구현
3. [ ] `feature/http-sse-layer` : Express 기반 HTTP(SSE) 전송 계층 연동 및 라우터 셋업
4. [ ] `feature/auth-middleware` : 환경변수(REDMINE_API_KEY, REDMINE_URL) 로드 및 인증 미들웨어(Header 위임) 구현
5. [ ] `test/e2e-integration` : Redmine 연동 엔드투엔드(E2E) 통합 테스트 수행
6. [ ] `docs/setup-guide` : Claude Desktop 및 Cursor IDE 연동 가이드 (setup_and_deployment.md) 작성
