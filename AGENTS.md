...
> [!WARNING] 에이전트 자발적 문서화 강제 규칙 (Self-Documenting Trigger)
> 에이전트는 코드 작성을 시작하기 전이나 진행 중에 다음 행위가 발생하면 **반드시 사용자의 지시가 없더라도 자발적으로 ADR 또는 DL을 작성**해야 합니다.
> 1. **패키지/의존성 추가**: `npm install` 등을 통해 새로운 외부 패키지나 라이브러리를 도입할 때 ➡️ **DL 작성**
> 2. **환경변수/설정 변경**: `.env`, `tsconfig.json`, 등 주요 환경 설정을 변경할 때 ➡️ **DL 작성**
> 3. **구조/아키텍처 변경**: 새로운 디렉토리 구조를 설계하거나, 통신 방식 및 데이터 흐름을 바꿀 때 ➡️ **ADR 작성**
> 4. **작업 계획(Task Plan) 수립**: 에이전트가 `todo.md` 또는 `task.md` 등에 새로운 기능 구현을 위한 작업 계획을 수립할 때, 1순위 체크리스트로 `[ ] 아키텍처 및 라이브러리 도입 관련 DL/ADR 작성 여부 판단` 항목을 무조건 포함해야 합니다.

---

## 7. 다중 에이전트 동시 작업 및 Git 워크플로우 (Git Worktree SOP)

프로젝트 내에서 단일 브랜치 충돌을 방지하고, 여러 에이전트(또는 사람)가 안전하게 동시 작업을 진행하기 위해 **Git Worktree 기반의 격리 환경 구성**을 강제합니다.
...
> - **격리된 작업 환경 필수**: 메인 에이전트는 문서와 환경설정만 다루고, 실제 코드 작성은 반드시 서브에이전트에게 위임(`invoke_subagent`)하여 진행해야 합니다.
> - **서브 에이전트 워크트리 강제**: 서브 에이전트를 호출할 때는 `Workspace` 파라미터를 반드시 `"share"` 또는 `"branch"`로 설정해야 합니다. (시스템 훅에 의해 `inherit`은 차단됩니다.)
> - **워크트리 경로 일원화**: 모든 워크트리는 메인 저장소 내부의 `.worktrees/` 디렉토리 하위로 관리되어 파편화를 방지합니다. 
> - **커밋 컨벤션 준수**: 모든 커밋은 Conventional Commits(예: `feat(scope): message`) 규격을 따라야 합니다.

---

## 8. TO-DO 대기열(Priority Queue) 관리 규칙

프로젝트의 진행 상황과 작업 순서는 `document/todo.md` 파일에 의해 관리됩니다 (Husky `pre-commit` 훅에서 경고 발생).

> [!CAUTION] 에이전트의 TO-DO 조작 시 필수 준수 사항
> 1. **무지성 하단 추가 금지**: 새로운 태스크나 버그 수정이 발생했을 때, `todo.md`의 맨 아래에 무작정 추가하지 마십시오. 기존 대기열 항목들과의 기술적 의존성(Dependency) 및 긴급도를 비교하여 **가장 합리적인 순서(위치)를 찾아 삽입**해야 합니다.
> 2. **사용자 승인(Review) 강제**: 에이전트가 독단적으로 대기열의 우선순위를 변경하거나 새 작업을 추가해서는 안 됩니다. 변경 전 반드시 사용자에게 "왜 이 위치(순서)에 작업을 추가/변경해야 하는지" 설명하고 **명시적인 승인(Approve)을 받은 후** 커밋하십시오.
> 3. **선형적 순서 권장 (Soft Constraint)**: 대기열 1순위 작업과 브랜치명을 일치시키는 것을 권장합니다. 단, 즉각적인 버그 수정이나 에이전트 동시 작업 등 유연성이 필요한 경우 경고(Warning)를 무시하고 진행할 수 있습니다.

---

## 9. AI 에이전트 행동 강령: 훅 무력화 금지 (--no-verify)

본 프로젝트는 Husky와 커스텀 스크립트를 통해 엄격한 TDD, 문서 동기화, 대기열(Queue) 관리를 강제하는 훅(Hook)을 운영하고 있습니다.

> [!CAUTION] 에이전트의 훅 우회 절대 금지
> 1. **`--no-verify` 사용 전면 금지**: 에이전트는 어떠한 경우에도 `git commit --no-verify` 명령을 사용하여 훅을 회피해서는 안 됩니다.
> 2. **에러의 정면 돌파**: 훅에서 에러(예: 커밋 메시지 규칙 위반, `todo.md` 대기열 누락 등)가 발생하여 커밋이 차단될 경우, 우회할 생각 대신 **훅이 제시하는 에러 로그를 읽고 원인(코드, 문서, 커밋 메시지)을 올바르게 수정하여 정당하게 훅을 통과**해야 합니다.
> 3. 시스템 가드레일은 에이전트를 괴롭히기 위함이 아니라 프로젝트의 무결성을 보호하기 위함임을 명심하십시오.

## 10. 에이전트 행동 철학: 선제적 탐색과 계획 (Proactive Planning)

모든 에이전트는 작업을 시작하기 전, 특히 파일 시스템 구조 변경이나 Git 커밋 등 프로젝트 규약이 얽힌 작업을 수행하기 전에 **관련된 제약 사항을 선제적으로 파악하고 계획**해야 합니다.

> [!IMPORTANT] 수동적 에러 수정(Reactive Fixing) 최소화
> 1. **관련 스크립트 선제적 열람**: 파일 생성이나 커밋을 수행하기 전, 가급적 연관된 훅 스크립트(`.husky/pre-commit`, `.husky/commit-msg` 등)와 가이드 문서를 `view_file` 도구로 확인하십시오.
> 2. **제약 사항 사전 매핑**: 스크립트 내부에 명시된 화이트리스트 경로(예: `document/decision_log/*`), 커밋 메시지 필수 태그(`[Impact-Reviewed]`), 샌드박스 제약 등 세부 조건을 작업 설계 단계에서 미리 반영(Plan-First)하여 한 번에 가드레일을 통과해야 합니다.
> 3. 에러에 부딪힌 뒤에야 뒤늦게 스크립트를 열어보고 파일을 재배치하는 행위는 토큰 및 시간 낭비의 주 원인이므로 반드시 지양해야 합니다.
